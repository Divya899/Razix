...
namespace Razix
{
    ...
    SceneManager::~SceneManager()
    {
        ...
        if(m_CurrentScene)
        {
            Razix_LOG_INFO("[SceneManager] - Exiting scene : {0}", m_CurrentScene->GetSceneName());
    ...
    void SceneManager::SwitchScene(const std::string& name)
    {
        ...
        {
            Razix_LOG_ERROR("[SceneManager] - Unknown Scene Alias : {0}", name.c_str());
        }
    }
    ...
    void SceneManager::ApplySceneSwitch()
    {
        ...
        {
            Razix_LOG_ERROR("[SceneManager] - Invalid Scene Index : {0}", m_QueuedSceneIndex);
            m_QueuedSceneIndex = 0;
        }
        ...
        {
            Razix_LOG_INFO("[SceneManager] - Exiting scene : {0}", m_CurrentScene->GetSceneName());
            app.GetSystem<RazixPhysicsEngine>()->SetPaused(true);
        ...
        app.GetSystem<RazixPhysicsEngine>()->SetDefaults();
        app.GetSystem<B2PhysicsEngine>()->SetDefaults();
        app.GetSystem<RazixPhysicsEngine>()->SetPaused(false);

        std::string physicalPath;
        if(Razix::VFS::Get()->ResolvePhysicalPath("//Scenes/" + m_CurrentScene->GetSceneName() + ".lsn", physicalPath))
        {
        ...
        Razix_LOG_INFO("[SceneManager] - Scene switched to : {0}", m_CurrentScene->GetSceneName().c_str());
    ...
    void SceneManager::EnqueueScene(Scene* scene)
    {
        ...
        Razix_LOG_INFO("[SceneManager] - Enqueued scene : {0}", scene->GetSceneName().c_str());
    }
