...
namespace Razix
{
    AudioData LoadOgg(const std::string& fileName)
    {
        ...
        if(!Razix::VFS::Get()->ResolvePhysicalPath(fileName, physicalPath))
        {
            Razix_LOG_INFO("Failed to load Ogg file : File Not Found");
        }
        ...
        if(!m_FileHandle)
        {
            Razix_LOG_CRITICAL("Failed to load OGG file '{0}'!", physicalPath);
        }
        int error;
        ...
        if(!m_StreamHandle)
        {
            Razix_LOG_CRITICAL("Failed to load OGG file '{0}'! , Error {1}", physicalPath, error);
            return data;
        }
