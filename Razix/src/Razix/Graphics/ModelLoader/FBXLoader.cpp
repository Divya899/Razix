...
namespace Razix::Graphics
{
    ...
    Maths::Vector2 ToRazixVector(const ofbx::Vec2& vec)
    {
        return Maths::Vector2(float(vec.x), float(vec.y));
    }

    Maths::Vector3 ToRazixVector(const ofbx::Vec3& vec)
    {
        return Maths::Vector3(float(vec.x), float(vec.y), float(vec.z));
    }

    Maths::Vector4 ToRazixVector(const ofbx::Vec4& vec)
    {
        return Maths::Vector4(float(vec.x), float(vec.y), float(vec.z), float(vec.w));
    }

    Maths::Vector4 ToRazixVector(const ofbx::Color& vec)
    {
        return Maths::Vector4(float(vec.r), float(vec.g), float(vec.b), 1.0f);
    }

    Maths::Quaternion ToRazixQuat(const ofbx::Quat& quat)
    {
    ...
    void Model::LoadFBX(const std::string& path)
    {
        ...
        if(data == nullptr)
        {
            Razix_LOG_WARN("Failed to load fbx file");
            return;
        }
        ...
        for(int i = 0; i < c; ++i)
        {
            ...
            if(material)
            {
                ...
                properties.albedoColour = ToRazixVector(material->getDiffuseColor());
                properties.metallicColour = ToRazixVector(material->getSpecularColor());
