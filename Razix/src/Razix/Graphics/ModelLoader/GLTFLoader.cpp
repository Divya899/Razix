...
namespace Razix::Graphics
{
    ...
    void LoadNode(Model* mainModel, int nodeIndex, const Maths::Matrix4& parentTransform, tinygltf::Model& model, std::vector<Ref<Material>>& materials, std::vector<std::vector<Graphics::Mesh*>>& meshes)
    {
        ...
        Razix_LOG_INFO("asset.copyright : {0}", model.asset.copyright);
        Razix_LOG_INFO("asset.generator : {0}", model.asset.generator);
        Razix_LOG_INFO("asset.version : {0}", model.asset.version);
        Razix_LOG_INFO("asset.minVersion : {0}", model.asset.minVersion);
#endif
    ...
    void Model::LoadGLTF(const std::string& path)
    {
        ...
        if(!warn.empty())
        {
            Razix_LOG_ERROR(warn);
        }

        if(!ret)
        {
            Razix_LOG_ERROR("Failed to parse glTF");
        }
        ...
        const tinygltf::Scene& gltfScene = model.scenes[Razix::Maths::Max(0, model.defaultScene)];
