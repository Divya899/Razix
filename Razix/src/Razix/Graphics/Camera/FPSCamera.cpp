...
namespace Razix
{
    ...
    void FPSCameraController::HandleKeyboard(Maths::Transform& transform, float dt)
    {
        ...
        if(Input::Get().GetKeyHeld(Razix::InputCode::Key::W))
        {
            m_Velocity += transform.GetForwardDirection() * m_CameraSpeed;
        }

        if(Input::Get().GetKeyHeld(Razix::InputCode::Key::S))
        {
            m_Velocity -= transform.GetForwardDirection() * m_CameraSpeed;
        }

        if(Input::Get().GetKeyHeld(Razix::InputCode::Key::A))
        {
            m_Velocity -= transform.GetRightDirection() * m_CameraSpeed;
        }

        if(Input::Get().GetKeyHeld(Razix::InputCode::Key::D))
        {
            m_Velocity += transform.GetRightDirection() * m_CameraSpeed;
        }

        if(Input::Get().GetKeyHeld(Razix::InputCode::Key::Space))
        {
            m_Velocity -= transform.GetUpDirection() * m_CameraSpeed;
        }

        if(Input::Get().GetKeyHeld(Razix::InputCode::Key::LeftShift))
        {
            m_Velocity += transform.GetUpDirection() * m_CameraSpeed;
        }
