...
namespace Razix
{
    namespace Graphics
    {
        ...
        struct Razix_EXPORT BasicVertex
        {
            Maths::Vector3 Position;
            Maths::Vector2 TexCoords;
        };

        struct Razix_EXPORT Vertex
        {
            Vertex()
...
{
    template <>
    struct hash<Razix::Graphics::Vertex>
    {
        size_t operator()(Razix::Graphics::Vertex const& vertex) const
        {
            return ((hash<Razix::Maths::Vector3>()(vertex.Position) ^ (hash<Razix::Maths::Vector2>()(vertex.TexCoords) << 1) ^ (hash<Razix::Maths::Vector4>()(vertex.Colours) << 1) ^ (hash<Razix::Maths::Vector3>()(vertex.Normal) << 1) ^ (hash<Razix::Maths::Vector3>()(vertex.Tangent) << 1)));
        }
    };
}
