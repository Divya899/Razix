...
namespace Razix
{
    namespace Graphics
    {
        ...
        uint8_t* GLTexture2D::LoadTextureData()
        {
            ...
            {
                uint32_t bits;
                pixels = Razix::LoadImageFromFile(m_FileName.c_str(), &m_Width, &m_Height, &bits, &isHDR, !m_LoadOptions.flipY);
        ...
        uint32_t GLTextureCube::LoadFromMultipleFiles()
        {
            ...
            bool isHDR = false;
            uint8_t* xp = Razix::LoadImageFromFile(xpos, &width, &height, &bits, &isHDR, true);
            uint8_t* xn = Razix::LoadImageFromFile(xneg, &width, &height, &bits, &isHDR, true);
            uint8_t* yp = Razix::LoadImageFromFile(ypos, &width, &height, &bits, &isHDR, true);
            uint8_t* yn = Razix::LoadImageFromFile(yneg, &width, &height, &bits, &isHDR, true);
            uint8_t* zp = Razix::LoadImageFromFile(zpos, &width, &height, &bits, &isHDR, true);
            uint8_t* zn = Razix::LoadImageFromFile(zneg, &width, &height, &bits, &isHDR, true);
        ...
        uint32_t GLTextureCube::LoadFromVCross(uint32_t mips)
        {
            ...
            {
                bool isHDR = false;
                uint8_t* data = Razix::LoadImageFromFile(m_Files[m], &srcWidth, &srcHeight, &bits, &isHDR, !m_LoadOptions.flipY);
                m_Parameters.format = BitsToTextureFormat(bits);
