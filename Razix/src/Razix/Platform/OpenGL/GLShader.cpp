...
namespace Razix
{
    namespace Graphics
    {
        ...
        void GLShader::Init()
        {
            ...
            if(!m_Handle)
            {
                Razix_LOG_ERROR("{0} - {1}", error.message[error.shader], m_Name);
            }
            else
            {
                Razix_LOG_INFO("Successfully compiled shader: {0}", m_Name);
            }
        ...
        void GLShader::SetUniform(ShaderDataType type, uint8_t* data, uint32_t size, uint32_t offset, const std::string& name)
        {
            ...
            if(m_UniformLocations.find(hashName) == m_UniformLocations.end())
            {
                ...
                {
                    Razix_LOG_WARN("Invalid uniform location {0}", name);
                }
            }
            ...
            if(location == -1)
            {
                Razix_LOG_ERROR("Couldnt Find Uniform In Shader: {0}", name);
                return;
            }
            ...
            case ShaderDataType::MAT4:
                ...
                break;
            default:
                Razix_ASSERT(false, "Unknown type!");
            }
        }
            ...
            for(uint32_t i = 0; i < lines.size(); i++)
            {
                ...
                else if(StringUtilities::StartsWith(str, "#include"))
                {
                    ...
                    if(strstr(file.c_str(), rem.c_str()))
                    {
                        ...
                        Razix_LOG_WARN("Including file \'{0}\' into shader.", file);
        ...
        uint32_t GLShader::Compile(std::map<ShaderType, std::string>* sources, GLShaderErrorInfo& info)
        {
            ...
            if(result == GL_FALSE)
            {
                ...
                Razix_LOG_ERROR(info.message[info.shader]);
                return 0;
            }
...
#endif
            default:
                Razix_LOG_ERROR("Unsupported Shader Type");
                return GL_VERTEX_SHADER;
            }
        }
        ...
        Shader* GLShader::CreateFuncGL(const std::string& filePath)
        {
            ...
            Razix::VFS::Get()->ResolvePhysicalPath(filePath, physicalPath);
