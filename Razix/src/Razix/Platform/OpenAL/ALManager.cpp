...
namespace Razix
{
    namespace Audio
    {
        ...
        void ALManager::OnInit()
        {
            ...
            Razix_LOG_INFO("Creating SoundSystem - {0}, Number of devices : ", alcGetString(m_Device, ALC_DEVICE_SPECIFIER), alcGetString(nullptr, ALC_DEVICE_SPECIFIER));

            if(!m_Device)
                Razix_LOG_INFO("Failed to create SoundSystem! (No valid device!)");
