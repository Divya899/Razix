...
#define VK_CHECK_RESULT(f)                                                                                                                    \
    {                                                                                                                                         \
        ...
        if(res != VK_SUCCESS)                                                                                                                 \
        {                                                                                                                                     \
            Razix_LOG_ERROR("[VULKAN] : VkResult is {0} in {1} at line {2}", Razix::Graphics::VKTools::ErrorString(res), __FILE__, __LINE__); \
        }                                                                                                                                     \
    }
...
namespace Razix
{
    namespace Graphics
    {
        ...
        namespace VKTools
        {
            ...
            VkFormat FormatToVK(Razix::Graphics::Format format);
            VkPolygonMode PolygonModeToVk(Razix::Graphics::PolygonMode mode);
