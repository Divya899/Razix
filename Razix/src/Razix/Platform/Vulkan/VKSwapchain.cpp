...
namespace Razix
{
    namespace Graphics
    {
        ...
        {
            Razix_ASSERT(m_CurrentBuffer < m_SwapchainBufferCount, "Incorrect swapchain buffer index");
        ...
        bool VKSwapchain::Init(bool vsync)
        {
            ...
            if(!m_Surface)
            {
                Razix_LOG_CRITICAL("[VULKAN] Failed to create window surface!");
            }
            ...
            if(queueIndexSupported == VK_FALSE)
                Razix_LOG_ERROR("Present Queue not supported");
        ...
        void VKSwapchain::AcquireNextImage()
        {
            ...
            {
                Razix_PROFILE_SCOPE("vkAcquireNextImageKHR");
                ...
                {
                    Razix_LOG_INFO("Acquire Image result : {0}", result == VK_ERROR_OUT_OF_DATE_KHR ? "Out of Date" : "SubOptimal");
                ...
                {
                    Razix_LOG_CRITICAL("[VULKAN] Failed to acquire swap chain image!");
                }
        ...
        void VKSwapchain::QueueSubmit()
        {
            ...
            {
                Razix_PROFILE_SCOPE("vkQueueSubmit");
        ...
        void VKSwapchain::Present()
        {
            ...
            {
                Razix_LOG_ERROR("[Vulkan] Swapchain out of date");
            }
            else if(error == VK_SUBOPTIMAL_KHR)
            {
                Razix_LOG_ERROR("[Vulkan] Swapchain suboptimal");
            }
            else
            {
