...
namespace Razix
{
    namespace Graphics
    {
        ...
        VKPhysicalDevice::VKPhysicalDevice()
        {
            ...
            if(numGPUs == 0)
            {
                Razix_LOG_CRITICAL("ERROR : No GPUs found!");
            }
            ...
            Razix_LOG_INFO("Vulkan : {0}.{1}.{2}", VK_VERSION_MAJOR(m_PhysicalDeviceProperties.apiVersion), VK_VERSION_MINOR(m_PhysicalDeviceProperties.apiVersion), VK_VERSION_PATCH(m_PhysicalDeviceProperties.apiVersion));
            Razix_LOG_INFO("GPU : {0}", std::string(m_PhysicalDeviceProperties.deviceName));
            Razix_LOG_INFO("Vendor ID : {0}", StringUtilities::ToString(m_PhysicalDeviceProperties.vendorID));
            Razix_LOG_INFO("Device Type : {0}", std::string(TranslateVkPhysicalDeviceTypeToString(m_PhysicalDeviceProperties.deviceType)));
            Razix_LOG_INFO("Driver Version : {0}.{1}.{2}", VK_VERSION_MAJOR(m_PhysicalDeviceProperties.driverVersion), VK_VERSION_MINOR(m_PhysicalDeviceProperties.driverVersion), VK_VERSION_PATCH(m_PhysicalDeviceProperties.driverVersion));
            ...
            Razix_ASSERT(queueFamilyCount > 0, "");
            ...
            if(extCount > 0)
            {
                ...
                {
                    Razix_LOG_INFO("Selected physical device has {0} extensions", extensions.size());
                    for(const auto& ext : extensions)
                    {
                        m_SupportedExtensions.emplace(ext.extensionName);
                        Razix_LOG_INFO("  {0}", ext.extensionName);
                    }
                }
            }
        ...
        uint32_t VKPhysicalDevice::GetMemoryTypeIndex(uint32_t typeBits, VkMemoryPropertyFlags properties) const
        {
            ...
            Razix_ASSERT(false, "Could not find a suitable memory type!");
            return UINT32_MAX;
        }
        /////////////////////////
...
#endif
#if defined(Razix_PROFILE) && defined(TRACY_ENABLE)
        ...
        bool VKDevice::Init()
        {
...
#if defined(Razix_PLATFORM_MACOS) || defined(Razix_PLATFORM_IOS)
            ...
            {
                Razix_LOG_CRITICAL("[VULKAN] vkCreateDevice() failed!");
                return false;
            }
            ...
            {
                Razix_LOG_CRITICAL("[VULKAN] Failed to create VMA allocator");
            }
#endif
