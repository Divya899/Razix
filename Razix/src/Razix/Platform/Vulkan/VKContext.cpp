...
namespace Razix
{
    namespace Graphics
    {
...
#if 0
#if defined(TRACY_ENABLE) && defined(Razix_PLATFORM_WINDOWS)
            ...
            void* userData)
        {
            ...
            {
                Razix_LOG_WARN("[VULKAN] - ERROR : [{0}] Code {1}  : {2}", pLayerPrefix, msgCode, pMsg);
            };
            // Warnings may hint at unexpected / non-spec API usage
            if(flags & VK_DEBUG_REPORT_WARNING_BIT_EXT)
            {
                Razix_LOG_WARN("[VULKAN] - WARNING : [{0}] Code {1}  : {2}", pLayerPrefix, msgCode, pMsg);
            };
            // May indicate sub-optimal usage of the API
            if(flags & VK_DEBUG_REPORT_PERFORMANCE_WARNING_BIT_EXT)
            {
                Razix_LOG_WARN("[VULKAN] - PERFORMANCE : [{0}] Code {1}  : {2}", pLayerPrefix, msgCode, pMsg);
            };
            // Informal messages that may become handy during debugging
            if(flags & VK_DEBUG_REPORT_INFORMATION_BIT_EXT)
            {
                Razix_LOG_WARN("[VULKAN] - INFO : [{0}] Code {1}  : {2}", pLayerPrefix, msgCode, pMsg);
            }
            // Diagnostic info from the Vulkan loader and layers
            // Usually not helpful in terms of API usage, but may help to debug layer and loader problems
            if(flags & VK_DEBUG_REPORT_DEBUG_BIT_EXT)
            {
                Razix_LOG_INFO("[VULKAN] - DEBUG : [{0}] Code {1}  : {2}", pLayerPrefix, msgCode, pMsg);
            }
        ...
        {
            Razix_PROFILE_FUNCTION();
#ifndef Razix_PLATFORM_IOS
            if(volkInitialize() != VK_SUCCESS)
            {
                Razix_LOG_CRITICAL("volkInitialize failed");
            }

            if(volkGetInstanceVersion() == 0)
            {
                Razix_LOG_CRITICAL("Could not find loader");
            }
#endif
            ...
            appInfo.engineVersion = VK_MAKE_VERSION(RazixVersion.major, RazixVersion.minor, RazixVersion.patch);
            ...
            {
                Razix_LOG_CRITICAL("[VULKAN] Validation layers requested, but not available!");
            }

            if(!CheckExtensionSupport(m_InstanceExtensionNames))
            {
                Razix_LOG_CRITICAL("[VULKAN] Extensions requested are not available!");
            }
            ...
            {
                Razix_LOG_CRITICAL("[VULKAN] Failed to create instance!");
            }
#ifndef Razix_PLATFORM_IOS
        ...
        void VKContext::SetupDebugCallback()
        {
            ...
            {
                Razix_LOG_CRITICAL("[VULKAN] Failed to set up debug callback!");
            }
        }
