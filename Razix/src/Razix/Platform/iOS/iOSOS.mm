...
static iOSOS* os = nullptr;
    ...
    void iOSOS::Init()
    {
        Razix::Internal::CoreSystem::Init(false);

        std::string root = GetAssetPath();
        Razix::VFS::Get()->Mount("CoreShaders", root + "Shaders");

       // Razix::VFS::Get()->Mount("Shaders", root + "shaders");
        Razix::VFS::Get()->Mount("Meshes", root + "Meshes");
        Razix::VFS::Get()->Mount("Textures", root + "Textures");
        Razix::VFS::Get()->Mount("Scripts", root + "Scripts");
        Razix::VFS::Get()->Mount("Scenes", root + "Scenes");
        Razix::VFS::Get()->Mount("Sounds", root + "Sounds");
        Razix::VFS::Get()->Mount("Assets", root + "Assets");

        Razix_LOG_INFO("Device : {0}",GetModelName());
    ...
    void iOSOS::OnQuit()
    {
        ...
        Application::Release();
	    Razix::Internal::CoreSystem::Shutdown();
    }
    ...
    {
        ((iOSWindow*)Application::Get().GetWindow())->OnKeyEvent((Razix::InputCode::Key)Razix::iOSKeyCodes::iOSKeyToRazix(keycode), down);
    }
...
#pragma mark - RazixMetalView

@interface RazixMetalView : MTKView <RazixView, MTKViewDelegate>
...
- (void)drawInMTKView:(MTKView *)view {
    ...
    else if (newDrawableWidth != self.drawableWidth || newDrawableHeight != self.drawableHeight)
    {
        ...
        Razix::os->OnScreenResize(self.drawableWidth, self.drawableHeight);
    }
...
@interface RazixAppDelegate : UIResponder <UIApplicationDelegate>
...
#pragma mark - RazixViewController

@interface RazixViewController : UIViewController<UIKeyInput, UITextInputTraits> {
...
@implementation RazixViewController
...
- (UIView<RazixView> *)RazixView {
    return (UIView<RazixView> *)self.view;
}
...
- (void)loadView
{
    ...
    UIView<RazixView> *RazixView = nil;
        
    if (self.metalDevice)
    {
        RazixView = [[[RazixMetalView alloc] initWithFrame:frame
    ...
    Razix::os->SetIOSView((__bridge void *)RazixView);
    Razix::os->SetWindowSize(frame.size.width * scale, frame.size.height * scale);
    Razix::os->Init();
}
...
- (void)viewDidLoad
{
    ...
    auto *delegate = (RazixAppDelegate*)UIApplication.sharedApplication.delegate;
    self.RazixView.animating = delegate.active;
    ...
    
    {
        ...
        switch(phase)
        {
            case TouchPhaseCancelled:
                Razix::iOSOS::Get()->OnScreenPressed(currLocation.x, currLocation.y, (uint32_t)index, false);
                break;
            case TouchPhaseBegan:
                Razix::iOSOS::Get()->OnScreenPressed(currLocation.x, currLocation.y, (uint32_t)index, true);
                break;
            case TouchPhaseEnded:
                Razix::iOSOS::Get()->OnScreenPressed(currLocation.x, currLocation.y, (uint32_t)index, false);
                break;
            case TouchPhaseMoved:
                Razix::iOSOS::Get()->OnMouseMovedEvent(currLocation.x, currLocation.y);
                break;
        }
...
- (void)insertText:(NSString *)text {
    //Razix::iOSOS::Get()->OnKeyPressed(text.UTF8String, false);
}

- (void)deleteBackward {
    Razix::iOSOS::Get()->OnKeyPressed('d', false);
}
...
- (void)keyPressed:(UIKeyCommand *)keyCommand {
    Razix::iOSOS::Get()->OnKeyPressed('d', true);
}
...
-(void) handleKeyboardInput: (unichar) keycode {
    Razix::iOSOS::Get()->OnKeyPressed(keycode, true);
}

-( void )layoutSubviews
{
    Razix::iOSOS::Get()->OnScreenResize(self.view.bounds.size.width * self.view.contentScaleFactor, self.view.bounds.size.height * self.view.contentScaleFactor);
}
...
@implementation RazixAppDelegate
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    Razix::iOSOS::Create();
    Razix::os = (Razix::iOSOS*)Razix::iOSOS::Instance();
    ...
    self.window.rootViewController = [[[RazixViewController alloc] init] autorelease];
...
- (void)setActive:(BOOL)active {
    if (_active != active) {
        ...
        RazixViewController *vc = (RazixViewController *)[self.window rootViewController];
...
- (void)applicationWillResignActive:(UIApplication *)application
{
    ...
    Razix::Input::Get().SetWindowFocus(false);
}
...
- (void)applicationWillEnterForeground:(UIApplication *)application
{
    ...
    Razix::Input::Get().SetWindowFocus(true);
}
...
- (void)applicationWillTerminate:(UIApplication *)application
{
    ...
    ((Razix::iOSOS*)Razix::iOSOS::Instance())->OnQuit();
    Razix::iOSOS::Release();
}
...
{
	@autoreleasepool {
        return UIApplicationMain(argc, argv, nil, NSStringFromClass([RazixAppDelegate class]));
    }
}
