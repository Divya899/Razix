...
namespace Razix
{
	...
	{
        Razix_LOG_INFO("Creating window - Title : {0}, Width : {1}, Height : {2}", properties.Title, properties.Width, properties.Height);
    ...
    void iOSWindow::OnKeyEvent(Razix::InputCode::Key key, bool down)
    {
        if(down)
        {
    ...
    void iOSWindow::OnTouchEvent(uint32_t xPos, uint32_t yPos, uint32_t count, bool down)
    {
        ...
        if(down)
        {
            MouseButtonPressedEvent* event2 = new MouseButtonPressedEvent((Razix::InputCode::MouseKey)Razix::iOSKeyCodes::iOSTouchToRazixMouseKey(count));
            m_QueuedEvents.push_back(event2);
        }
        else
        {
            MouseButtonReleasedEvent* event2 = new MouseButtonReleasedEvent((Razix::InputCode::MouseKey)Razix::iOSKeyCodes::iOSTouchToRazixMouseKey(count));
            m_QueuedEvents.push_back(event2);
