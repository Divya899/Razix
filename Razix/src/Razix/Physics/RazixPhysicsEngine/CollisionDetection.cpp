...
namespace Razix
{
    CollisionDetection::CollisionDetection()
    {
        ...
        Razix_LOG_INFO("Max Collision Size {0}", m_MaxSize);
    ...
    bool CollisionDetection::CheckCollision(RigidBody3D* obj1, RigidBody3D* obj2, CollisionShape* shape1, CollisionShape* shape2, CollisionData* out_coldata)
    {
        ...
        if((shape1->GetType() | shape2->GetType()) >= m_MaxSize)
            Razix_LOG_INFO("Invalid collision func {0}, {1}, {2}, {3}", shape1->GetType(), shape1->GetType(), shape2->GetType() | shape2->GetType(), m_MaxSize);
    ...
    {
        Razix_LOG_CRITICAL("Invalid Collision type specified");
        return false;
    }
