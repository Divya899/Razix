...
namespace Razix
{
    ...
    float RazixPhysicsEngine::s_UpdateTimestep = 1.0f / 60.0f;

    RazixPhysicsEngine::RazixPhysicsEngine()
        : m_IsPaused(true)
        , m_UpdateAccum(0.0f)
    ...
    void RazixPhysicsEngine::OnUpdate(const TimeStep& timeStep, Scene* scene)
    {
        ...
        if(!m_IsPaused)
        {
            ...
            {
                Razix_PROFILE_SCOPE("Physics::Get Rigid Bodies");
            ...
            {
                Razix_PROFILE_SCOPE("Physics::Get Spring Constraints");
                m_Constraints.clear();
            ...
            {
                Razix_PROFILE_SCOPE("Physics::Get Axis Constraints");
            ...
            {
                Razix_PROFILE_SCOPE("Physics::Get Distance Constraints");
            ...
            {
                Razix_PROFILE_SCOPE("Physics::Get Weld Constraints");
            ...
            {
                Razix_PROFILE_SCOPE("Physics::UpdatePhysics");
                if(m_MultipleUpdates)
                {
                    ...
                    {
                        Razix_LOG_WARN("Physics too slow to run in real time!");
            ...
            {
                Razix_PROFILE_SCOPE("Physics::Set Transforms");
    ...
    void RazixPhysicsEngine::UpdatePhysics()
    {
        m_Manifolds.clear();
    ...
    void RazixPhysicsEngine::UpdateRigidBodys()
    {
#ifdef THREAD_RIGID_BODY_UPDATE
        Razix_PROFILE_SCOPE("Thread Update Rigid Body");
...
#else
        Razix_PROFILE_SCOPE("Update Rigid Body");
    ...
    void RazixPhysicsEngine::UpdateRigidBody(RigidBody3D* obj) const
    {
        Razix_PROFILE_FUNCTION();
    ...
    void RazixPhysicsEngine::BroadPhaseCollisions()
    {
        Razix_PROFILE_FUNCTION();
    ...
    void RazixPhysicsEngine::NarrowPhaseCollisions()
    {
        Razix_PROFILE_FUNCTION();
    ...
    void RazixPhysicsEngine::SolveConstraints()
    {
        Razix_PROFILE_FUNCTION();

        {
            Razix_PROFILE_SCOPE("Solve Manifolds");

            for(Manifold& m : m_Manifolds)
                m.PreSolverStep(s_UpdateTimestep);
        }
        {
            Razix_PROFILE_SCOPE("Solve Constraints");
        ...
        {
            Razix_PROFILE_SCOPE("Apply Impulses");
    ...
    void RazixPhysicsEngine::ClearConstraints()
    {
        m_Constraints.clear();
    }
    ...
    void RazixPhysicsEngine::OnImGui()
    {
        Razix_PROFILE_FUNCTION();
    ...
    void RazixPhysicsEngine::OnDebugDraw()
    {
        Razix_PROFILE_FUNCTION();
